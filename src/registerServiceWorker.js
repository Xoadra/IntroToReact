	...
	if ( process.env.NODE_ENV === 'production' && 'serviceWorker' in navigator ) {
		...
		window.addEventListener( 'load', ( ) => {
			...
			if ( isLocalhost ) {
				...
				// Point developers to the service worker/PWA documentation
				...
				// Is not local host so just register service worker
				...
				installingWorker.onstatechange = ( ) => {
					if ( installingWorker.state === 'installed' ) {
						if ( navigator.serviceWorker.controller ) {
							...
							// It's the perfect time to display a message in your web app
							console.log( 'New content is available; please refresh.' )
						} else {
							// At this point, everything has been precached
							// It's the perfect time to display another message
	...
	fetch( swUrl )
		.then( response => {
			...
			if (
				...
				// Probably a different app, so reload the page
